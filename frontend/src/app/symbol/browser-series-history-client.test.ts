// `T-05.2` — `browser-series-history-client.ts`'s own transport tests, same shape
// `series-history-client.test.ts` uses: `HistoryPageFetchErrorKind` is exercised with a mocked
// `fetchImpl` (synthetic `Response`/`Error`), never a real socket and never a real Next server.

import assert from "node:assert/strict";
import { test } from "node:test";

import {
  browserHistoryRequestPath,
  fetchSeriesHistoryFromBrowser,
  HistoryPageFetchError,
  SERIES_HISTORY_PROXY_PATH,
} from "./browser-series-history-client.ts";
...
test("browserHistoryRequestPath is RELATIVE — no origin, no base URL, ever", () => {
  const path = browserHistoryRequestPath(KEY);
  assert.equal(path.startsWith(SERIES_HISTORY_PROXY_PATH + "?"), true);
  assert.equal(path.startsWith("http"), false);
  assert.match(path, /series_key_id=abc123/);
  assert.match(path, /bar_policy=final_only/);
});

test("CALA: a well-formed proxy response resolves to the parsed envelope", async () => {
  const response = new Response(JSON.stringify(validEnvelopeBody()), { status: 200 });
  const envelope = await fetchSeriesHistoryFromBrowser(KEY, { fetchImpl: fakeFetch(response) });
  assert.equal(envelope.rows.length, 1);
  assert.equal(envelope.rows[0]?.value, "42.5");
});

test("MORDE: a non-2xx proxy response throws HistoryPageFetchError kind=non_2xx with the real status", async () => {
  const response = new Response(JSON.stringify({ error: "boom" }), { status: 422 });
  await assert.rejects(
    () => fetchSeriesHistoryFromBrowser(KEY, { fetchImpl: fakeFetch(response) }),
    (error: unknown) => {
...
test("MORDE: a network failure throws HistoryPageFetchError kind=connection_refused", async () => {
  ...
  await assert.rejects(
    () => fetchSeriesHistoryFromBrowser(KEY, { fetchImpl: failingFetch }),
    (error: unknown) => {
      assert.ok(error instanceof HistoryPageFetchError);
