...
| **custo declarado** | exige **definir e medir** o termo "atraso de publicação do endpoint" separado da fase — hoje a evidência é o **mínimo** sob fase uniforme (`12 ms`, `n = 4.289`), que é estimador de mínimo, **não** de `p99`. Para um `p99` de publicação é preciso instrumentação diferente (ex.: poll denso alinhado, ou usar o `premiumIndex` como referência — `p99 = 1.758 ms`, `n = 34.752`, `min = −100 ms`. ⚠️ **Correção de 2026-09-11:** este documento afirmava aqui que o coletor de `premiumIndex` era *"alinhado à grade"*. **É falso** — ele fecha o ciclo com o mesmo `stop_event.wait(interval_s)` pós-trabalho, e a deriva mede `60,441 s/ciclo` (`34.752 ÷ 8 linhas/ciclo = 4.344` ciclos em `262.496 s`) `[MEDIDO 2026-09-11 sobre os próprios campos de `publication_lag_table.py`]`. O `p99` baixo **não** vem de alinhamento: para este endpoint `bucket_end = source_time` (`use_cases/collector_series_mapping.py:232`), logo `available_at − bucket_end = received_at − source_time` e a fase do poll é **algebricamente ausente da coluna** — o número seria o mesmo sob um escalonador perfeito e sob um que derivasse uma hora por dia. E o `min = −100 ms` **não pode** vir do horário do poll (recebemos sempre DEPOIS do carimbo deles): é **desvio de relógio**). ⚠️ E note: **`O5` é `O4` sem o conserto** — mede o que `O4` construiria, mas deixa a produção torta |
