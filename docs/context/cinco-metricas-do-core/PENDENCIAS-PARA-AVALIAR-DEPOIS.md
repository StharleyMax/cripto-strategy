...
| A11 | **`/symbol` degrada por latência e cada painel novo piora** — `page.tsx:305` passou de **4 para 6** leituras concorrentes contra uma API que **serializa**; o spec mede **`2` de `6`** renders com `panel_absent:connection_refused`, varredura serializada de `81,1 s`. Rota custa **`245 s`** com 6 séries contra **`139 s`** com 4 `[MEDIDO 2026-09-16]` | `frontend/src/app/symbol/page.tsx:296-311`; `gates/CODE-REVIEW-FASE-05.md` `W-2` | causa-raiz é [`ACHADO-API-VAZA-IDLE-IN-TRANSACTION.md`](gates/ACHADO-API-VAZA-IDLE-IN-TRANSACTION.md) (`B12`/`B13`), **não** esta fase. Merece **task própria antes de virar erro de tela** — hoje o e2e recarrega até 3× só por este motivo |
