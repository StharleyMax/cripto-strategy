...
| `T-03.2` | `[sentimento][infra]` Use case `list_series_catalog()` e rota `GET {API_PREFIX}/series-catalog` | concatena as linhas reais de `cvd_source_catalog`/`price_source_catalog`/`open_interest_catalog` (**3+2+5=10**, **não** as **7** contagens de call-site `SeriesCatalogEntry(` — corrigido em `T-03.2` ciclo 2, `[MEDIDO 2026-09-07]`: `open_interest_catalog` expande 1 call-site em 5 linhas, `CA-F2-17` proíbe colapsar); envelope `{"query":"series_catalog","n_entries":10,"entries":[…]}` com os nomes de campo do TS `series-catalog.ts:134`; `Completeness` **não** vai no fio; **0** SQL no handler; DI em `dependencies.py`; pytest | `D3.1`: `curl … series-catalog | python3 -c '…print(d["n_entries"],len(d["entries"]))' → 10 10`; **≠** `grep -rn 'SeriesCatalogEntry(' backend/src … | wc -l` (esse é **7**, call-sites); `grep -rn SELECT backend/src/api | wc -l → 0`. Ausente: `000` | `T-03.1`, `T-02.2` | `US-9`, `RF-9`, `CA-F3-1`, `NG-3` (`4c` fora) |
| `T-03.3` | `[web]` `parseCatalogEnvelope` e S3 exibe as 10 linhas da API | parser compõe `assertValidCatalogEntry` + `QuarantineTerms` + `Completeness: unmeasured`; estrito em campo ausente (`ADR-019/D2`); `FIXTURE_CATALOG_ROWS` fora do grafo de produção; teste TS (1 campo removido por vez) | e2e: S3 `catalog_rows = 10` (corrigido de 7 em 2026-09-07 junto com `T-03.2`). **Ausente: `error_kind:connection_refused`, `catalog_rows = 0`; `FIXTURE_CATALOG_ROWS` visível ⇒ reprova** | `T-03.2`, `T-01.4` | `US-9`, `CA-F3-1`, `RN-3` |
