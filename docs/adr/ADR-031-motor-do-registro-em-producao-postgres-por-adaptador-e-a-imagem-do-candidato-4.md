...
**Data:** 2026-09-07 · **Status:** **proposta** pelo `/architect` — nasce com a `SPEC-004` (`SPEC_DRAFT`); vale quando o owner der `approve spec` em `captura-em-producao` (o ledger é a identidade do estado, não este cabeçalho). **Co-assinatura recomendada:** `quant-architect` (dono de `sentimento` — o registro é dele) e `infra-architect` (a imagem e o serviço `postgres` são `infra`).
