...
**Data:** 2026-09-07 · **Status:** **proposta** pelo `/architect` — nasce com a `SPEC-004` (`SPEC_DRAFT`); vale quando o owner der `approve spec` em `captura-em-producao` (o ledger é a identidade do estado, não este cabeçalho). **Co-assinatura recomendada:** `quant-architect` (dono de `sentimento` — o registro é dele) e `infra-architect` (a imagem e o serviço `postgres` são `infra`). **Co-assinatura do `quant-architect` satisfeita em 2026-09-07** — [`gates/Q3-run-definition.md`](../context/captura-em-producao/gates/Q3-run-definition.md) (§7) decide o que `D3` grava; o flip formal de `Status` para `aceita` é `T-01.9`, não este parágrafo.
