...
> **Nota, 2026-09-26: o código de tick foi removido. A porta continua aberta, mas quem a reabre reescreve.** `aggtrade_csv_reader`, `aggtrade_contiguity`, `aggtrade_bucket_aggregate` e `domain/cvd.py` saíram do repositório, junto com os testes deles, no commit `chore(infra): Postgres único por sessão, remoção do aggTrades morto…` (branch `chore/harness-velocidade`). Nenhum deles tinha chamador em `backend/src`, e o CVD de produção vem de klines (`kline_cvd.py`, `ADR-036/D5`). Os testes que sobravam releiam 8,87M linhas de CSV de terceiro a cada execução (~179 s de uma suíte de 451 s) para afirmar fatos sobre os arquivos, não sobre código em uso `[MEDIDO 2026-09-26: pytest --durations=60]`. Os números que eles fixavam continuam em §0.3.3 e no `data/MANIFEST.md`. `[DECISÃO-OWNER: 2026-09-26, escolha entre alternativas apresentadas — "testes + código morto" contra "só os testes pesados"]`. **Se `Q20` trouxer absorção por tamanho de trade**, o ponto de partida é `git show <sha>^:backend/src/modules/sentimento/infra/aggtrade_csv_reader.py`, e a medição dos arquivos vira script em `backend/scripts/`, não teste da suíte.
