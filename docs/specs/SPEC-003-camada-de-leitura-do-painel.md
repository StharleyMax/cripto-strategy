...
| 7 | `SeriesCatalogEntry(` constantes: **7** em 3 módulos | `grep -rn 'SeriesCatalogEntry(' backend/src --include='*.py' \| grep -v test \| wc -l` | **7** (`cvd_source_catalog`, `price_source_catalog`, `open_interest_catalog`) | confere — **mas é contagem de call-sites, não de linhas do catálogo**: `open_interest_catalog.py` expande 1 call-site em 5 linhas por `CA-F2-17` ⇒ catálogo real = **10** (`T-03.2`, `[MEDIDO 2026-09-07]`, ver §3.6/§5 B14) |
...
|---|---|---|---|
| **catálogo** | `GET {API_PREFIX}/series-catalog` | `{"query":"series_catalog","n_entries":10,"entries":[SeriesCatalogEntry…]}` — `n_entries` é `len(entries)`, computado, nunca literal; `SeriesCatalogEntry` no fio = os campos do tipo TS `series-catalog.ts:134` (mesmos nomes; `Completeness` **não** vai no fio — o front preenche `unmeasured`); handler com **0** linha de SQL — concatena as linhas dos 3 módulos de catálogo já populados (`cvd_source_catalog` 3 + `price_source_catalog` 2 + `open_interest_catalog` 5 = **10**; `CA-F2-17` proíbe colapsar as 4 reduções da Coinalyze em 1) via use case `list_series_catalog()` | **corrigido em `T-03.2` (`[MEDIDO 2026-09-07]`): `n_entries=10`, não `7` — `7` era contagem de call-sites de `SeriesCatalogEntry(`, não de linhas reais; ver §0.1#7 e §5 B14** |
...
| B14 | (`F3`) `GET /series-catalog` | `n_entries = len(entries) = 10` (`[MEDIDO 2026-09-07]`: `curl -s $API$PREFIX/series-catalog \| python3 -c 'import json,sys;d=json.load(sys.stdin);print(d["n_entries"],len(d["entries"]))'` → `10 10`); **não** `= grep -c 'SeriesCatalogEntry('` (esse grep dá **7** call-sites — `open_interest_catalog.py` expande 1 call-site em 5 linhas, `CA-F2-17`) | `000`; S3 mostra erro, 0 linhas | S3 com linhas sem API |
