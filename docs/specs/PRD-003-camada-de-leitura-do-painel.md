...
| id | story | fronteira | aceite |
|---|---|---|---|
| **US-9** | Como operador, vejo no S3 o **catálogo real de séries** (as 10 entradas que hoje só existem em código — corrigido de "7" em `T-03.2`, `[MEDIDO 2026-09-07]`: `open_interest_catalog` expande 1 dos 7 call-sites `SeriesCatalogEntry(` em 5 linhas, `CA-F2-17`, então `3+2+5=10` linhas reais, não 7 call-sites). | use case `list_series_catalog()` concatenando as linhas reais dos 3 módulos (`quant-architect`); rota `GET {prefix}/series-catalog` (`infra`); `parseCatalogEnvelope` compondo `assertValidCatalogEntry` + `QuarantineTerms` + `Completeness: unmeasured` (`web`) | `CA-F3-1` |
...
| id | critério | comando (cala) | **servidor ausente (morde)** |
|---|---|---|---|
| CA-F3-1 | Catálogo servido = 10 linhas reais (corrigido de "7" em `T-03.2`, `[MEDIDO 2026-09-07]`) | `curl $API$PREFIX/series-catalog \| python3 -c 'import json,sys;print(len(json.load(sys.stdin)["entries"]))'` → **10**; **≠** `grep -rn 'SeriesCatalogEntry(' backend/src --include='*.py' \| grep -v test \| wc -l` (esse é **7**, call-sites — `open_interest_catalog` expande 1 em 5 linhas, `CA-F2-17` proíbe colapsar); S3 exibe **10** linhas; handler com `0` linha de SQL (`D5.13c`) | `curl → 000`; S3 exibe erro/sem fonte, **0** linhas |
