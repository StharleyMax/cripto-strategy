...
| 2 | o arquivo tocado **já estava no perímetro de auditoria** da task, e a travessia o **APERTA**, nunca o afrouxa | quem responde por não afrouxar um arquivo tem autoridade para apertá-lo, **e o aperto foi medido COMO aperto**: o `select` vai de **231 para 275** regras habilitadas — as **44** novas são todas `D` `[MEDIDO 2026-08-29, controle invertido, ruff PINADO do projeto: com `"D"` → 275 habilitadas / 44 delas `D`; sem `"D"` → 231 / **0** `D`]`. **⚠️ Correção do `/review` 2026-08-29: a primeira redação citava aqui o "275 antes e depois, `--show-settings` byte a byte idêntico" — essa medição existe e está certa, mas ela é sobre **com e sem a linha `ignore`**, e prova que o `ignore` não afrouxa; ela NÃO é o antes/depois da travessia.** A condição 2 fala da travessia, e o número que a prova é o `+44` acima — que é **mais forte** que a célula anterior. `[NOTA DE MÉTODO: o TOTAL absoluto varia com a versão do `ruff` (o `/review` mediu 267/223 com o binário do `PATH`); a DIFERENÇA, `+44`, é idêntica nos dois. Publique o delta, nunca o total.]` |
