...
|---|---|---|---|---|
| 3.1 | Use case `list_series_catalog()` concatenando as linhas reais de `cvd_source_catalog`/`price_source_catalog`/`open_interest_catalog` (**3+2+5 = 10**, não as **7** contagens de call-site `SeriesCatalogEntry(` — `open_interest_catalog` expande 1 call-site em 5 linhas, `CA-F2-17` proíbe colapsar; corrigido em `T-03.2`, `[MEDIDO 2026-09-07]`); rota `GET {API_PREFIX}/series-catalog` com envelope `{"query":"series_catalog","n_entries":10,"entries":[…]}` (`n_entries = len(entries)`, computado); **0** SQL no handler; DI stub em `dependencies.py` | `US-9`, `RF-9` | `sentimento` (use case) + `infra` (rota) | não calcula `Completeness` (`4c`, `NG-3`) |
| 3.2 | `parseCatalogEnvelope` no front compondo `assertValidCatalogEntry` + `QuarantineTerms` + `Completeness: unmeasured`; S3 exibe as **10** linhas (item 3.1, `[MEDIDO 2026-09-07]`, era **7** por engano); estrito em campo ausente (`ADR-019/D2`) | `US-9` | `web` | não usa `FIXTURE_CATALOG_ROWS` |
...
| **D3.1** catálogo = 10 (`[MEDIDO 2026-09-07]`, corrigido de **7** — ver item 3.1) | `curl -s $API$PREFIX/series-catalog \| python3 -c 'import json,sys;d=json.load(sys.stdin);print(d["n_entries"],len(d["entries"]))'` → `10 10`; **≠** `grep -rn 'SeriesCatalogEntry(' backend/src --include='*.py' \| grep -v test \| wc -l` (esse é **7**, call-sites — `open_interest_catalog` expande 1 em 5 linhas, `CA-F2-17`); `grep -rn 'SELECT' backend/src/api \| wc -l` → **0**; e2e: S3 `catalog_rows = 10` | `curl → 000`; S3 exibe `error_kind:connection_refused`, `catalog_rows = 0`; **`FIXTURE_CATALOG_ROWS` visível ⇒ reprova** |
